...
//        when(jsoupWrapper.getHtml(urlToScrape)).thenReturn(mockDocument);
        Mockito.when(carCategoryMetadataService.isFirstPage(urlToScrape)).thenReturn(false);
        Mockito.when(categoryCooldownHandler.hasCooldown(urlToScrape)).thenReturn(false);
